...
<script>
	...
	var tabDataGeoaquifere = document.getElementById('tab_data_geoaquifere'); // contenant des données collectées par AJAX		
	...
	function affiche_geo_aquifere_data()
	{
		...
		xhr.onreadystatechange = function() 
		{
			if (xhr.readyState === 4 && xhr.status === 200) 
			{
				...
				tab_geo_aquifere = jsonResponse['tab_geo_aquifere'];
				message_info = jsonResponse['message_info'];

				if(tab_geo_aquifere)
				{
					htmlcode = jsonResponse['htmlcode'];				
					tabDataGeoaquifere.innerHTML = htmlcode; // Ajoute la ligne dans le tableau des fichiers importables
				}
				else
				{
	...
	affiche_geo_aquifere_data(); // Lancement de la fonction permettant d'aller chercher les données de types de chroniques
	...
	function delete_aquifere(id_aquifere)
	{
		...
		xhr.onreadystatechange = function() 
		{
			if (xhr.readyState === 4 && xhr.status === 200) 
			{
				...
				del_aquifere = jsonResponse['del_aquifere'];
				message_info = jsonResponse['message_info'];
