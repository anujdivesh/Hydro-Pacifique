...
<script>
	...
	var tabDataGeoCommune = document.getElementById('tab_data_geocommune'); // contenant des données collectées par AJAX		
	...
	function affiche_geo_commune_data()
	{
		...
		xhr.onreadystatechange = function() 
		{
			if (xhr.readyState === 4 && xhr.status === 200) 
			{
				...
				tab_geo_commune = jsonResponse['tab_geo_commune'];
				message_info = jsonResponse['message_info'];

				if(tab_geo_commune)
				{
					htmlcode = jsonResponse['htmlcode'];				
					tabDataGeoCommune.innerHTML = htmlcode; // Ajoute la ligne dans le tableau des fichiers importables
				}
				else
				{
	...
	affiche_geo_commune_data(); // Lancement de la fonction permettant d'aller chercher les données de types de chroniques
	...
	function delete_commune(id_commune)
	{
		...
		xhr.onreadystatechange = function() 
		{
			if (xhr.readyState === 4 && xhr.status === 200) 
			{
				...
				del_commune = jsonResponse['del_commune'];
				message_info = jsonResponse['message_info'];
