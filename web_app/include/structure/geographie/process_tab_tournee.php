...
$sql_tournee = "SELECT DISTINCT id, nom, description 
                    FROM ".TABLE_TOURNEE." 
...
{	
    ...
    $del_tournee = true;
    $sql_tournee_station = "SELECT id_station FROM ".TABLE_STATION_TO_TOURNEE."
                        WHERE id_tournee=".$id_tournee."
                        LIMIT 1";
    $tournee_station_query = tep_db_query($sql_link,$sql_tournee_station);  
    $tournee_station = tep_db_fetch_array($tournee_station_query);
    if(isset($tournee_station)){$del_tournee = false;}    

    $tournee_array[$id_tournee] = array('nom_tournee' => $nom_tournee,
                                        'description_tournee' => $description_tournee,  
...
$htmlcode .= "<div class='table-container' style='float:left;height:70vh;'>";
    ...
    $htmlcode .= "<table id='table_tri' cellspacing='0' style=''>";
        ...
        $htmlcode .= "<tr><td colspan='3' style='color:#000;font-size:14px;font-weight:bold;'>".htmlaccent('Ajouter une Tournée')."</td></tr>\n";
        ...
        {   
            ...
            {				
                ...
                $htmlcode .= "<tr ".$row_l." id='row_rh_".$id."' >";
                    ...
                    $htmlcode .= "<td>";
                        $htmlcode .= "<input type='text' style='width:250px;' name='tournee_nom_".$id."' value='".$data['nom_tournee']."'>\n";
                    $htmlcode .= "</td>";

                    $htmlcode .= "<td>";
                        $htmlcode .= "<input type='text' style='width:450px;' name='tournee_description_".$id."' value='".$data['description_tournee']."'>\n";
                    $htmlcode .= "</td>";
                    ...
                    $htmlcode .= "<td style='text-align:center;'>";
                        ...
                        if($data['del_tournee'])
                        {
                            $htmlcode .= "<span class='del' title='".htmlaccent('Supprimer')."' onClick=\"delete_tournee('".$id."');\">";
                                $htmlcode .= "X";
                            $htmlcode .= "</span>";
                        }
