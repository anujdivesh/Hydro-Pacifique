...
----------------------------------------
Importation des données LAB (Pluviométrie) - Formùat Spécifique à la NC, 
...
$startTime = microtime(true); // Temps au début du script
    ...
    if($import_data['file_ext'] == 'csv')
    {
        ...
        {   
            ...
            while (($data = fgetcsv($handle, 10000, ';')) !== false) 
            {
                ...
                // Colonne 3 (Total -> lié à un cumul 99999 donc à une lacune)
                $data_total='null';
                if($import_valid)
                {
                    if(isset($data[2]))
                ...
                if($import_valid)
                {
                    ...
                    if(isset($data[3]))
                    {
                        if(isset($quality_data_array[$data[3]])) // si l'intitulé du code qualité est enregistrée dans la base
                        {
    ...
    if(isset($data_tab) && sizeof($data_tab))
    {
        ...
        try
        {
            ...
            $sql_delete_data = "DELETE FROM ".TABLE_DATA_LAB." WHERE id_station = ".$import_data['id_station'].
            ...
            $query_insert_bloc_data = "INSERT INTO ".TABLE_DATA_LAB." (id_station, date_heure, cumul, total, id_data_qualite, obs) VALUES ";

            $rows = []; // Tableau pour stocker les données à insérer
