...
$initQualiteLookup = array(); // ça permettra de vérifier que le code qualité n'est pas déjà actif
...
if($id_eq_type == 1) // uniquement pour les stations Pluvio
{
	...
	while($lab_tab = tep_db_fetch_array($lab_query))
	{
		...
		if(!isset($initQualiteLookup[$code_qual_encours]) && ($code_qual_encours <> 'L')) // Pour ne pas afficher les données qualifiées en lacunes
		{
		...
		if($code_qual_encours <> 'L')
		{
			...
			$data_graph .= "
							...
							{ 
								...
								marker: 
										{
											size: 5, // Ajustez la taille des marqueurs selon vos besoins
	...
	while($etl_tab = tep_db_fetch_array($etl_query))
	{
		...
		$data_graph .= "
						...
						{ 
							...
							hovermode: 'closest',
							hovertemplate: '<b>ETL</b>: %{text}  <br><b>Date</b>: %{x|%d-%m-%Y}',
							text: ['Début','Fin'],
...
$html_tab_data_station .= "<table id='table_tri' cellspacing='0'>";
    ...
    $html_tab_data_station .= 
                        "
                            <thead>
                                <tr>
                                    <th style='width:90px;font-size:12px;'>".htmlaccent('Chroniques')."</th>					
                                    <th style='width:60px;font-size:12px;'>".htmlaccent('Nb data')."</th>
                                    <th style='width:75px;font-size:12px;'>".htmlaccent('Date début')."</th>
                                    <th style='width:75px;font-size:12px;'>".htmlaccent('Date fin')."</th>
                                </tr>
                            </thead>
                        ";
