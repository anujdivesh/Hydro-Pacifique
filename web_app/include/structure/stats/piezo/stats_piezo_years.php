...
while($data = tep_db_fetch_array($data_query))
{
	...
	$data_piezo[$date_mesure] = array('year_m' => $year_mesure,'heure_m' => '00:00:00','hauteur' => $data['hauteur'],'hauteur_lac' => $data['hauteur_lac'],'lacune' => $data['lacune_day']);
	
	if($data['hauteur'] > $max_piezo){$max_piezo = $data['hauteur_lac'];}
	if($nb_data==0){$min_piezo=$max_piezo;}
	
	if($data['hauteur'] < $min_piezo){$min_piezo = $data['hauteur_lac'];}
...
if($nb_data>0)
{
	...
	foreach($data_piezo as $cle => $element)
	{
		...
		{
			if($nb_days==0){$means_data=0;}
			else{$means_data = round($last_tot_data/$nb_days,2);}
...
echo "<h1><span>";
	...
	echo "<span>".htmlaccent('Tableau de bord - Moyenne quotidienne des relevés piézométriques')."</span>";
	if($nb_data > 0 && !$print)
	{	
		echo button_print('print_stats_years.php?ty=piezo&print=ok&bs=1&il='.$select_region.'&st='.$select_station.'&eq=2','Tableau');
	}
echo "</h1>"; 
...
echo "<div id='box_graph_all'>";
	...
	echo "<div id='box_graph' class='lgt_r' >";
		...
		echo "<div id='container_all' style='float:left;width:95%;height:200px;margin-left:3%;'></div>";
						...
						foreach($year_tab_inverse as $cle => $element)
						{
							if($element['means']<0)
							{
								...
								echo "<div id='box_graph' class='".$col_style."' style='margin-right:1%;'>";
									...
									echo "<div id='container_".$cle."' style='float:left;width:95%;height:300px;margin-left:3%;'></div>";
...
function()
{
	...
	var options0 = {
					 ...
					 'yaxis': {'showLabels': true, 'trackDecimals': 0, 'min':<?php echo $min_piezo; ?>, 'max': <?php echo $max_piezo; ?>}, 
					 'lines': {'show': true, 'lineWidth': 1.5, 'fill': true, 'fillOpacity': 0}, 
					 ...
					 'mouse': {'track': true, 'trackAll': true, 'position': 'nw', 'relative': true, 'margin': 20, 'trackDecimals': 2, 'trackFormatter':function dateTracker(obj){
																																									  ...
																																									  if(sec<10){sec='0'+sec;}
																																									  return 'Hauteur d\'eau moyenne : ' + obj.y + ' m<br>Année : '+fullYear; 
																																								  } 
					 			}
	...
	var options1 = {
					 ...
					 'mouse': {'track': true, 'trackAll': true, 'position': 'nw', 'relative': true, 'margin': 20, 'trackDecimals': 2, 'trackFormatter':function dateTracker(obj){
																																									  ...
																																									  var sec = dateToDisplay.getSeconds();
																																									  if(sec<10){sec='0'+sec;}
																																									  return 'Hauteur d\'eau : ' + obj.y + ' m<br>Date: '+date+'-'+month+'-'+fullYear; 
																																								  }
